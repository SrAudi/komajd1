	...
	public boolean onCommand(CommandSender sender, Command cmd, String lbl, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("responder")) {
			if(!p.hasPermission("kom.ajd")) {
				p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Voce não tem permissao");
				return true;
			}
			
			 if(args.length == 0 ) {
  			   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Digite /responder (jogador) resposta");
  			   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Exemplo: /responder ZidenVentania O Nível Maximo é 100");
  			   return true;
  		   }
			...
			players.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 O Ajudante : §a" + p.getName() );
			players.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Respondeu : §a" + mensagem );
			players.sendMessage(" ");
			String logs = "&a O Ajudante &b"+p.getName()+" &arespondeu o Jogador &b"+players.getName()+"$es &aresposta:&6 "+mensagem+" !";
			   p.sendMessage(" ");
			   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Você respondeu o jogador §a" + reporter.toLowerCase());
			   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Sua resposta foi §a" + mensagem);
			   Comando.reports.set("Player."+reporter.toLowerCase(), null);
			   Comando.reports.saveConfig();
			   for(Player plsa : Bukkit.getOnlinePlayers()) {
				  if(plsa.hasPermission("kom.staff")) {
				   plsa.sendMessage(" ");
				   plsa.sendMessage("§c§l[§6§lK§e§lo§6§lM§c§l] >>§c O Ajudante §a"+p.getName()+"§c respondeu §a"+reporter.toLowerCase());
				   plsa.sendMessage("§c§l[§6§lK§e§lo§6§lM§c§l] >>§c Resposta: §a"+mensagem);
			   }
			  }
			   log.set("Player."+as, logs);
			   log.saveConfig();
				  players.getWorld().playSound(players.getLocation(), Sound.ENTITY_ENDERMEN_TELEPORT, 1, 1);
				   ...
				   /* Abaixo entre ifs são o sistema de contagem de duvidas respondidas por cada ajudante*/
			   int pontos = ajudante.getConfig().getInt("Ajudante."+ajd);
			   	int total = pontos +1 ;
			   if(ajudante.getConfig().getString("Ajudante."+ajd) == null){
				   /* se não tiver o player na config ajudante, seta 1 */
