...
//	public static T_Config pts = new T_Config(Main.getPlugin(Main.class), "pts.yml");
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lb, String[] args) {
		...
		if(!(sender instanceof Player)) {
			Bukkit.getConsoleSender().sendMessage("§a§l[KoM AJD] §7 apenas jogadores podem usar esse comando!");
			return true;
		}
...
//    		   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§c§l Acesso Negado apenas jogadores");
//    	   return true;
//    	   }
    	   if(args.length == 0 ) {
    			p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Digite /duvida (sua duvida)");
    			p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Exemplo:§a /duvida 'qual nível maximo?' ");
    			return true;
    	   }	
    	   if(reports.getConfig().getString("Player."+p.getName().toLowerCase()) != null ) {
			   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§c§l Você ja reportou aguarde uma resposta!");
			   return true;
    	   ...
    	   if (args.length > 0) {
    		   ...
    		   for(Player players : Bukkit.getOnlinePlayers()) {
    			   ...
    			   if(players.hasPermission("kom.ajd") || p.isOp()) {
    		   ...
    		   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Sua duvida foi publicada!");
    		   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Sua duvida: §a"+mensagem);
    		   reports.saveConfig();
