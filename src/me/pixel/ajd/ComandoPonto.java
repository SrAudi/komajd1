...
public class ComandoPonto implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lbl, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("darponto")) {
			if(!p.hasPermission("kom.staff")) {
 			   p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Você não tem permissão !");
				return true;
			}
			if(args.length == 0) {
				p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Digite /darponto <ajudante>");
				p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Exemplo: /darponto Jabu");

				return true;
			}
			
		if(args.length == 1) {
			Player pl = Bukkit.getPlayer(args[1]); /* var que diz q o args é player */
			if(!pl.hasPermission("kom.ajd")) {
		   ...
		   if(pts.getConfig().getString("Pontos."+pl.getName()) == null){
			   /* se não tiver o player na config ajudante, seta 1 */
				p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Você deu §1§7 ponto para §a"+pl.getName());
		...
		if(args.length == 2) {
			Player pls = Bukkit.getPlayer(args[1]);
			 ...
			 if(pts.getConfig().getString("Pontos."+pls.getName()) == null){
				   /* se não tiver o player na config ajudante, seta 1 */
					p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Você deu "+arg+" ponto para §a"+pls.getName());
		...
		if(args.length > 0) {
			Player pl = Bukkit.getPlayer(args[0]); /* var que diz q o args é player */

			int pontos = pts.getConfig().getInt("Pontos."+pl.getName());
		   	int total = --pontos ;
		   if(pts.getConfig().getInt("Pontos."+pl.getName()) == 0){
			   /* se não tiver o player na config ajudante, seta 0 */
				p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 O Ajudante não possui pontos!");
		   ...
		   if(pts.getConfig().contains("Pontos."+pl.getName())){
				p.sendMessage("§7§l[§6§lK§e§lo§6§lM§7§l] >>§7 Você tirou §1§7 ponto para §a"+pl.getName());
	...
	if(cmd.getName().equalsIgnoreCase("verpontos")) {
		if(!p.hasPermission("kom.staff")) {
			p.sendMessage("§c§l[§6§lK§e§lo§6§lM§c§l] >>§c Você não tem permissão");
		...
		if(pls == null) {
			p.sendMessage("§c§l[§6§lK§e§lo§6§lM§c§l] >>§c Jogador não encontrado!");
